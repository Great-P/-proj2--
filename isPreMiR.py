...
def second_struct_predict(seq):
    seq_struct = []
    os.system('.\\bin\\ViennaRNA\\RNAfold.exe < .\\temp\\temp_sequence.fa --noPS \
               > .\\temp\\temp_seq_struct.fa')
...
def predict_results(seq_struct_vector):
    ...
    # model = load_model('src/RNN/RNN_model_proj2_rnn.h5')
    # model = load_model('src/RNN/RNN_model.h5')
    # model = load_model('src/CNN/CNN_model.h5')
    # prediction
...
def main(argv):
    ...
    if len(seq) > 0:
        ...
        # print("seq_struct_vector shape:", np.shape(seq_struct_vector))
        # print("seq_struct_vector lenth:", len(seq_struct_vector))
